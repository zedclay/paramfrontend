...
const DownloadDetail = () => {
  ...
  const fetchDownload = async () => {
    try {
      setLoading(true);
      setError(null);
      const response = await axios.get(`/public/downloads/${id}`);
    ...
    return (
      <div className="container mx-auto px-4 py-16">
        <div className="text-center">
          ...
          <p className="text-gray-600 mb-8">{error || 'Cette téléchargement n\'existe pas ou n\'est plus disponible.'}</p>
          <Link
            to="/downloads"
  ...
  const title = download.title?.[locale] || download.title?.fr || download.title;
  const content = download.content?.[locale] || download.content?.fr || download.content;
                  ...
                  <div className="flex items-center gap-4 text-sm text-gray-500">
                    ...
                    {download.author && (
                      <div className="flex items-center">
                        ...
                        <span>{download.author.name}</span>
                      </div>
                    )}
                  </div>
                </div>
            ...
            <div className="flex items-center gap-4 mb-6 p-4 bg-gray-50 rounded-lg">
              {download.image_path && (
                ...
                >
                  ...
                  <span>{download.image_filename || 'Image'}</span>
                </a>
              )}
              {download.file_path && (
                <a
                ...
                >
                  ...
                  <span>{download.file_filename || 'PDF'}</span>
            ...
            <div className="flex flex-wrap gap-3">
              {download.target_audience && (
                <span className="px-3 py-1 bg-blue-50 text-blue-700 rounded-full text-sm">
                  {download.target_audience === 'all'
                    ? 'Public : Tous'
                    : download.target_audience === 'students'
                    ? 'Public : Étudiants'
